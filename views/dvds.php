...
<section class="panel panel-default">
    ...
    <form id="form" class="filters panel-body " action="<?= $_SERVER['PHP_SELF']; ?>" method="post">
        <section class="row">
            <fieldset class="col-xs-4">
                ...
                <input class="form-control" type="text" name="title" id="title"
                       value="<?php if(isset($_POST['title'])) {echo $_POST['title'];} ?>" onchange="submitForm()"/>
            </fieldset>
            <fieldset class="col-xs-4">
                <label for="genre">Genre:</label>
                <select name="genre" class="form-control" id="genre" onchange="submitForm()">
                    <?php foreach($view->genres as $genre): ?>
                        <option <?php if(isset($view->selected['genre']) && $view->selected['genre'] === $genre) {echo 'selected ';}?>
            ...
            <fieldset class="col-xs-4">
                ...
                <select name="director" class="form-control" id="director" onchange="submitForm()">
                    <?php foreach($view->directors as $director): ?>
                        <option <?php if(isset($view->selected['director']) && $view->selected['director'] === $director) {echo 'selected ';}?>
        ...
        <section class="row">
            <fieldset class="col-xs-4">
                ...
                <input class="form-control" type="text" name="actor" id="actor"
                       value="<?php if(isset($_POST['actor'])) {echo $_POST['actor'];} ?>" onchange="submitForm()"/>
            </fieldset>
            <fieldset class="col-xs-4">
                ...
                <select name="year_from" class="form-control" id="year_from" onchange="submitForm()">
                    <?php foreach($view->years as $year): ?>
                        <option <?php if(isset($view->selected['year_from']) && $view->selected['year_from'] === $year) {echo 'selected ';}?>
            ...
            <fieldset class="col-xs-4">
                ...
                <select name="year_to" class="form-control" id="year_to" onchange="submitForm()">
                    <?php foreach($view->years as $year): ?>
                        <option <?php if(isset($view->selected['year_to']) && $view->selected['year_to'] === $year) {echo 'selected ';}?>
                            value="<?= $year; ?>"><?= $year; ?></option>
                    <?php endforeach; ?>
                </select>
            </fieldset>
            ...
            <fieldset class="col-xs-4">
                ...
                <select name="price" class="form-control" id="price" onchange="submitForm()">
                    <?php foreach($view->prices as $price): ?>
                        <option <?php if(isset($view->selected['price']) && $view->selected['price'] === $price) {echo 'selected ';}?>
        ...
        <section class="row">
            <div class="col-xs-12">
                <button name="clear" type="submit" class="btn btn-primary centre">Clear</button>
            </div>
        </section>
    </form>
</section>
...
<?php foreach($view->films as $film): ?>
    <section class="well well-sm gallery-section">
        <figure style="padding:0 5px;">
            <a href="<?= $film['url']; ?>">
                <img src="/images/dvds/<?= $film['picture']; ?>" width="152px"/>
            </a>
        ...
        <form method="post" action="<?= $_SERVER['PHP_SELF']; ?>" class="add-form">
            ...
            <button name="add" type="submit" class="btn btn-primary"
                <?php if(!$film['rentable']) {echo 'disabled';}?>>
                <?= ($film['rentable']) ? 'Add to Basket' : 'Unavailable' ?></button>
        </form>
    </section>
<?php endforeach; ?>
<script>
